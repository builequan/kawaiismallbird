  ...
  useEffect(() => {
    ...
    fetch('/data/affiliate-links/products-index.json')
      ...
      .then(productsData => {
        ...
        // Match the affiliate links from content with products (if any links exist)
        if (affiliateLinks.length > 0) {
          for (const link of affiliateLinks) {
            ...
            // Stop when we have 6 products
            if (matchedProducts.length >= 6) break
          }
        }

        // Always try to get 6 products
        if (matchedProducts.length >= 6) {
          setProducts(matchedProducts.slice(0, 6))
          setLoading(false)
        }
        // If not enough products, fill with popular bird products
        else {
          let finalProducts = [...matchedProducts]

          // Get popular bird products
          if (finalProducts.length < 6) {
            const popularProducts = productsData
              .filter((p: any) => !finalProducts.find(fp => fp.id === p.id))
              .filter((p: any) => p.product_name.includes('鳥') || p.product_name.includes('インコ') || p.product_name.includes('オウム') || p.product_name.includes('文鳥') || p.product_name.includes('セキセイ'))
              .slice(0, 6 - finalProducts.length)
