...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      processed: posts.docs.length,
      updated: updatedCount,
      errors: errors,
      message: `Successfully removed reference sections from ${updatedCount} posts`
    })
  } catch (error: any) {
    console.error('Error removing reference sections:', error)
    return NextResponse.json(
      {
        success: false,
        message: error.message || 'Failed to remove reference sections'
      },
      { status: 500 }
    )
  }
