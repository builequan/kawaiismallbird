 ...
 * - No self-linking
 * - No bidirectional linking (if A→B, then B won't link to A)
 * - Each target linked only once per post
 * - Natural phrase selection from content
 */
...
function applyLinksToContent(
  ...
  if (newContent.root.children && Array.isArray(newContent.root.children)) {
    // Process nodes from end to beginning to maintain text positions
    for (let nodeIndex = newContent.root.children.length - 1; nodeIndex >= 0; nodeIndex--) {
      const node = newContent.root.children[nodeIndex]
      
      if (node.type !== 'paragraph' || !node.children) continue
      
      // Process text nodes from end to beginning
      for (let childIndex = node.children.length - 1; childIndex >= 0; childIndex--) {
        const child = node.children[childIndex]
        
        if (child.type !== 'text' || !child.text) continue
        
        // Try to apply each link to this text node
...
async function applySemanticLinks(options: { limit?: number; force?: boolean } = {}) {
  ...
  console.log('🧠 Applying semantic internal links (no bidirectional)...')
  ...
  try {
    ...
    for (const post of posts) {
      try {
        ...
        if (!force && postIndex && post.internalLinksMetadata?.contentHash === postIndex.contentHash) {
        ...
        console.log(`  ✅ Saved with ${linksAdded.length} links`)
        successCount++
    ...
    console.log('\n' + '='.repeat(50))
    console.log('✅ Semantic linking complete (no bidirectional links)!')
    console.log(`📊 Results:`)
    ...
    console.log(`  - Average links per post: ${(totalLinks / establishedLinks.size).toFixed(2)}`)
